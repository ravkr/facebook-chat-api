    ...
    switch (p["__typename"]) {
      ...
      case "ReducedMessagingActor":
        return {
          ...
          name: p.name,
          url: p.url, // in this case it is null all the time
...
function formatThreadList(data) {
  return data.map((t) => {
    return {
      ...
      nicknames: t.customization_info?t.customization_info.participant_customizations.map((u) => { return {"userID": u.participant_id, "nickname": u.nickname }; }):[], // TODO: simplify?
      muteUntil: t.mute_until,
      participants: formatParticipants(t.all_participants),
      adminIDs: t.thread_admins.map(a => { return a.id; }), // feature from future? it is always an empty array - for more than a year (2018-03-22)
      folder: t.folder,
      isGroup: t.thread_type === "GROUP",
      // rtc_call_data: t.rtc_call_data, // TODO: format and document this
      // isPinProtected: t.is_pin_protected, // feature from future? always false (2018-03-22)
      // customizationEnabled: t.customization_enabled, // TODO: always true? (was true even when customization_info was null) (2018-03-22)
      // participantAddModeAsString: t.participant_add_mode_as_string, // "ADD" if "GROUP" and null if "ONE_TO_ONE". do we need it? it may be connected with adminIDs
...
module.exports = function(defaultFuncs, api, ctx) {
  return function getThreadList(limit, timestamp, tags, callback) { // TODO: if no tags, use tags as callback
    ...
    defaultFuncs
      ...
      .then((resData) => {
        require("fs").writeFile("threadList_raw_clear.json", JSON.stringify(resData), (err) => { if (err) throw err; });
